...
router.post("/sharp", upload.single("originalImg"), async (req, res, next) => {
  try {
    ...
    const lgMediaFileName = `${filename.slice(0,filename.length-4)}.lg.${lgMediaWidth}x${lgMediaHeight}.${mimeToExt[mimetype]}`;
    const mdMediaFileName = `${filename.slice(0,filename.length-4)}.md.${mdMediaWidth}x${mdMediaHeight}.${mimeToExt[mimetype]}`;
    const smMediaFileName = `${filename.slice(0,filename.length-4)}.sm.${smMediaWidth}x${smMediaHeight}.${mimeToExt[mimetype]}`;
    const xsMediaFileName = `${filename.slice(0,filename.length-4)}.xs.${xsMediaWidth}x${xsMediaHeight}.${mimeToExt[mimetype]}`;
